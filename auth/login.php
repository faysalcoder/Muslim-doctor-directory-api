...
if (!is_array($data)) error_response('Invalid JSON payload', 400);
...
$pdo  = db();
$stmt = $pdo->prepare('SELECT id, name, email, password, role FROM admins WHERE email = :email AND status = :status LIMIT 1');
