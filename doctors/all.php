...
// Fetch doctors — NO JSON_ARRAYAGG (not available in older MySQL versions)
...
// Fetch gallery images separately to avoid JSON_ARRAYAGG
if (!empty($doctors)) {
    ...
    $imgStmt->execute($doctorIds);
    $allImages = $imgStmt->fetchAll();

    // Group images by doctor_id
    $imageMap = [];
